  ...
  transform(students: Student[], searchCriteria:string=""): Student[] {
    ...
    for(let student of students){
      if(
        ...
        || student.name.toLowerCase().includes(criteria)
        || student.sat.toString().includes(criteria)
        || student.gpa.toString().includes(criteria)
        || student.majorId != null && student.major.name.toLowerCase().includes(criteria)
      ) {
